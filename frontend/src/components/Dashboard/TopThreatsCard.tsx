...
const TopThreatsCard: React.FC<TopThreatsCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      {/* Content */}
      <div className="p-6">
        {topCVEs.filter((cve) => cve.cve_id && cve.cve_id.trim() !== "")
          .length === 0 ? (
          <div className="text-center py-8">
            ...
            <div className="text-sm text-gray-500 mt-1">This could mean:</div>
            <div className="text-xs text-gray-400 mt-2 space-y-1">
              <div>• No vulnerabilities have been scanned yet</div>
              <div>• Vulnerabilities found don't have CVE identifiers</div>
              <div>• All findings are non-CVE related issues</div>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {topCVEs
              .filter((cve) => cve.cve_id && cve.cve_id.trim() !== "")
              .slice(0, 10)
              .map((cve, index) => (
                <div
                  key={cve.cve_id}
        ...
        {/* Footer */}
        {(() => {
          const validCVEs = topCVEs.filter(
            (cve) => cve.cve_id && cve.cve_id.trim() !== ""
          );
          return (
            validCVEs.length > 0 && (
              <div className="mt-6 pt-6 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    Showing top {Math.min(validCVEs.length, 10)} CVEs
                  </div>
                  <div className="text-sm text-gray-600">
                    Total unique CVEs: {validCVEs.length}
                  </div>
                </div>
              </div>
            )
