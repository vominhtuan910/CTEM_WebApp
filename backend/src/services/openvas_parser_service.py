...
class OpenVASParserService:
    ...
    def parse_openvas_reports(self, specific_files: List[str] = None) -> List[Dict]:
        """
        Parse OpenVAS XML reports
        Args:
            specific_files: Optional list of specific file paths to parse
        Returns:
            List of threats with required fields
        """
        threats = []
        ...
        if specific_files:
            ...
            # Parse all files in directory (existing behavior)
    ...
    def get_threats_summary(self, threats: List[Dict]) -> Dict:
        ...
        # Count by status
        status_counts = {"Fixed": 0, "Not Fixed": 0, "In Progress": 0}
